...
const TransitList = () => {
  ...
  useEffect(() => {
    const fetchTransit = async () => {
      ...
      setError('');
      try {
        const response = await fetch(`${API_BASE_URL}/api/tobeshipped/list?status=shipped`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
        if (!response.ok) {
          const err = await response.json();
          throw new Error(err.message || 'Failed to fetch transit orders.');
        }
        const data = await response.json();
        setOrders(data);
      } catch (err) {
        setError(err.message || 'Error fetching transit orders. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="loading-container">
        ...
        <p className="loading-text">Loading Transit...</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 className="page-title">Transit</h1>
        <p className="page-subtitle">{orders.length} orders in transit</p>
      </div>

      {orders.length === 0 ? (
        <div className="admin-card">
          <div className="empty-state">
            <div className="empty-state-icon"><FontAwesomeIcon icon={faTruck} /></div>
            <div className="empty-state-title">No orders in transit</div>
            <div className="empty-state-text">Orders will appear here once marked as shipped from Ready to ship</div>
          </div>
        </div>
      ) : (
                  ...
                  <tr>
                    ...
                    <td>
                      <div className="table-actions" style={{ justifyContent: 'flex-end', gap: '0.625rem' }}>
                        <button onClick={() => handleMarkDelivered(order._id)} className="admin-btn" title="Mark as Delivered">Mark as Delivered</button>
